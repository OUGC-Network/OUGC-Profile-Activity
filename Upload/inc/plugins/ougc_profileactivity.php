...
function ougc_profileactivity_install()
{
    ...
    if ($PL->version < $info['pl_version']) {
        flash_message(
            $lang->sprintf($lang->ougc_profileactivity_plold, $PL->version, $info['pl_version'], $info['pl_url']),
            'error'
        );
        admin_redirect('index.php?module=config-plugins');
    }
...
function ougc_profileactivity()
{
    ...
    if ($thread_limit && $ttc) {
        ...
        $query = $db->simple_select(
            'threads t LEFT JOIN ' . TABLE_PREFIX . 'posts p on (t.firstpost=p.pid)',
            't.*, p.message',
            't.uid=\'' . $uid . '\' AND t.closed NOT LIKE \'moved|%\' AND t.visible=\'1\'' . $where,
            $order_options
        );
        ...
        while ($thread = $db->fetch_array($query)) {
            ...
            $thread['fullsubject'] = $thread['subject'] = htmlspecialchars_uni(
            ...
            // Format Time.
            $thread['dateline'] = $lang->sprintf(
                $lang->ougc_profileactivity_dateline,
    ...
    if ($posts_limit && $ptc) {
        ...
        while ($post = $db->fetch_array($query)) {
            ...
            // Format Time.
            $post['dateline'] = $lang->sprintf(
                $lang->ougc_profileactivity_dateline,
                my_date($mybb->settings['dateformat'], $post['dateline']),
                my_date($mybb->settings['timeformat'], $post['dateline'])
            );
